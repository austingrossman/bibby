...
// Frontend writes duty1/duty2/simulate_zc/frontend_iteration; heater-controller writes the rest.
typedef struct {
  ...
  ATOMIC(uint32_t) frontend_iteration;  // incremented each UI frame; heater-controller zeroes duties if stale
} HeaterShm;
