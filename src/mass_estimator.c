...
#define MASS_MAX_P_SPREAD 0.05f   // max std(P)/mean(P) for "constant power"
...
static void window_stats(const MassEstimator *e, double *slope_c_per_s,
                         float *p_mean, float *p_std, float *t_rise,
                         double *t_span) {
  int oldest = (e->head - e->count + MASS_EST_CAP) % MASS_EST_CAP;
  double t0 = e->buf[oldest].t;

  double sx = 0, sy = 0, sxx = 0, sxy = 0, sp = 0, spp = 0;
  float temp_min = 0, temp_max = 0;
  for (int i = 0; i < e->count; i++) {
    const MassSample *s = &e->buf[(oldest + i) % MASS_EST_CAP];
    double x = s->t - t0;
    double y = s->temp;
    sx  += x;   sy  += y;
    sxx += x*x; sxy += x*y;
  ...
  *t_rise = temp_max - temp_min;
  int newest = (e->head - 1 + MASS_EST_CAP) % MASS_EST_CAP;
...
void mass_estimator_push(MassEstimator *e, double t_s, float temp_filt_c,
  ...
  int oldest = (e->head - e->count + MASS_EST_CAP) % MASS_EST_CAP;
  while (e->count > 2 && t_s - e->buf[oldest].t > MASS_WINDOW_S) {
    e->count--;
    oldest = (oldest + 1) % MASS_EST_CAP;
  }

  if (t_s - e->last_eval_t < MASS_EVAL_DT_S || e->count < 20) return;
  e->last_eval_t = t_s;
  ...
  if (t_span < MASS_WINDOW_S * 0.9) return;               // window not full yet
  if (p_mean < e->min_power_w) return;                    // too little power
  if (p_std > MASS_MAX_P_SPREAD * p_mean) return;         // power not constant
  if (t_rise < MASS_MIN_RISE_C || slope <= 0.0) return;   // rise too small

  // Thermal mass P/slope in J/degC, expressed as litres of water.
  float litres = (float)(p_mean / slope) / WATER_C_J_PER_KG_C;
  // Smooth accepted estimates so one window cannot yank the gain schedule.
  e->estimate_l = (e->estimate_l > 0.0f) ? 0.7f * e->estimate_l + 0.3f * litres : litres;
}
